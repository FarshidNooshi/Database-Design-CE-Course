...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 17, 2021 at 09:14 AM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_hashtag_of_ava` (IN `sign` CHAR(6), IN `avaID` INT(11), IN `time` TIMESTAMP)  BEGIN
    ...
    SELECT avaID as ava_id, username as ava_sender_user, id, time as attached_time
    FROM special_sign
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Get_List_Of_Senders` ()  BEGIN 
    ...
    SET username = get_Last_Logged_In_User();
        ...
        FROM messages
        WHERE messages.to_id = username AND 
        messages.to_id NOT IN (SELECT blocks.blocked_id
                               FROM blocks
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `send_message` (IN `attributes` VARCHAR(256), IN `to_id` VARCHAR(20), IN `text` VARCHAR(128))  BEGIN

	IF text != '' THEN
    	CALL SendMessageWithText(text, to_id);
    END IF;
    IF text = '' THEN
    	CALL SendMessageWithAva(attributes, to_id);
    END IF;
...
('zeus', '2021-07-16 20:46:07', 'afasdf', 18),
('zeus', '2021-07-16 20:46:50', 'asdfasd', 19),
('zeus100', '2021-07-16 20:57:36', 'asdfasdfasdfsadf', 20),
...
('zeus', '2021-07-16 20:46:07', 18),
('zeus', '2021-07-16 20:46:50', 19),
('zeus100', '2021-07-16 20:57:36', 20);
...
('zeus100', '2021-07-16 20:57:24', 'log_in'),
('zeus100', '2021-07-16 20:58:08', 'log_in'),
('zeus100', '2021-07-16 20:58:52', 'log_in'),
('zeus100', '2021-07-16 21:04:10', 'log_in');
